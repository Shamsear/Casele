...
export function ModelPageClient({ products, displayName, modelSlug }: ModelPageClientProps) {
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
      <Reveal>
        <div className="mb-8">
          <p className="text-xs font-medium text-gold tracking-widest uppercase mb-2">Compatible Collection</p>
          <h1 className="font-display text-3xl font-bold text-white md:text-4xl">
            Cases for {displayName}
          </h1>
          <p className="mt-2 text-warm-gray leading-relaxed">
            <span className="text-white font-semibold">{products.length}</span> compatible {products.length === 1 ? "case" : "cases"} engineered for a precision fit
          </p>
      ...
      {products.length === 0 && (
        <Reveal>
          <div className="py-20 text-center">
            ...
            <p className="text-lg font-semibold text-white">No cases found for this model yet</p>
            <p className="mt-1 text-sm text-warm-gray">Browse other phone models or view all cases</p>
            <a
              href="/shop"
